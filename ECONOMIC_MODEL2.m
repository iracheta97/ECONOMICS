function [REA, WT] = ECONOMIC_MODEL2(WF,WT,PEL,ED)
...
PLOAD=ED.PLOAD/100;
if CTY==2
    if LL==1                      % Installation:
        LOAD=PLOAD*LOAD;          % kW
    else                          % Load = Generation
        LOAD=PLOAD*WF.N*PNET;     % kW
    end
    PELERATE='Venta=PML, Compra=GDMTH';
end
if CTY==3
    if LL==1                      % Installation:
        LOAD=PLOAD*LOAD;          % kW
...
for i=1:NWT+1
    ...
    % Type of contract (CTY):
    if CTY==1       % Total Sell:
    GRIDG(i,:)=(sum(EPS.*PGROSS*(i-1)))/F;           % USD
    GRIDN(i,:)=(sum(EPS.*PNET*(i-1)))/F;             % USD
    GRIDC(i,:)=(sum((EPS+CEL).*PNET*(i-1)))/F;       % USD
    LOAD=0*LOAD;
    elseif CTY==2   % Net billing:
    GRIDG(i,:)=(sum((EPS.*PGROSS*(i-1)-EPB.*LOAD)))/F;         % USD
    GRIDN(i,:)=(sum((EPS.*PNET*(i-1)-EPB.*LOAD)))/F;           % USD
    GRIDC(i,:)=(sum(((EPS+CEL).*PNET*(i-1)-(EPB).*LOAD)))/F;   % USD
    elseif CTY==3   % Net metering:
    % Grid Purchases (kWh/yr);
    % Change to (+) the load because thiis represents savings.
    [xgp ygp]=find(GPG<0);
    [xnp ynp]=find(GPN<0);
    % Grid Sales (kWh/yr):
    [xgs ygs]=find(GPG>0);
    [xns yns]=find(GPN>0);
    GRIDG(i,:)=sum((mean(EPS(xgs)).*GPG(xgs)))/F-sum((EPB(xgp).*GPG(xgp)))/F+sum((EPB.*LOAD))/F;       % USD
    GRIDN(i,:)=sum((mean(EPS(xns)).*GPN(xns)))/F-sum(EPB(xnp).*GPN(xnp))/F+sum((EPB.*LOAD))/F;       % USD
    GRIDC(i,:)=sum((mean(EPS(xns)+CEL)).*GPN(xns))/F-sum(EPB(xnp).*GPN(xnp))/F+sum((EPB.*LOAD))/F;   % USD
    end
    % Total Fuel Cost:
    FUEL(i,:)=0;                                             % USD
...
xlabel('Número de Años','fontsize',18,'fontname','tahoma','FontAngle','normal','fontweight','normal',...
    'HorizontalAlignment','center');
h=legend([PELERATE ' ( ' num2str(PEL.yy,'%.0f') ' )'],'Location','southeast');
...
xlabel(CAT1,'fontsize',18,'fontname','tahoma','FontAngle','normal','fontweight','normal',...
    'HorizontalAlignment','center');
h=legend([PELERATE ' ( ' num2str(PEL.yy,'%.0f') ' )'],'Location','northeast');
%h = legend('THD_v','Location','northeast','Orientation','horizontal');
%set(h,'LineWidth',2,'fontsize',14);
grid;
figure(np+3)
...
'HorizontalAlignment','center');
 ...
 'HorizontalAlignment','center');
  ...
  'HorizontalAlignment','center');
   ...
   grid
%--------------------------------------------------------------------------
% Analysis of Wind production and Load Consumption:
%--------------------------------------------------------------------------
yr=year(date);                          % Actual year
% Timestamp:
date11=datenum(yr,01,01,0:23,0,0);      % Date 1,
date22=datenum(yr,12,31,0:23,0,0);      % Date 2,
out1=datevec(date11':date22','yyyy-mm-dd HH:MM:SS');
[s1 s2]=size(out1);
% Number of days:
NDAYS=s1;
for i=1:s1
    ii=24*(i-1)+1;
    OUT1(ii:24*i,:)=datevec(date11+i-1);
end
WGM=[];WNM=[];LDM=[];
% Minimum length of vectors:
[x1 x2]=min(ff);
for ii=1:12
    % Extract month:
    [xm, ym]=find (OUT1(:,2)==ii);
    [xmm, ymm]=find (xm<x1);
    % Montly Wind Production (kWh/yr):
    WGM(ii)=(NWT)*sum(PGROSS(xm(xmm)))/F;                         % kWh/yr
    WNM(ii)=(NWT)*sum(PNET(xm(xmm)))/F;                           % kWh/yr
    % Monthly load profile:
    LDM(ii)=sum(LOAD(xm(xmm)))/F;                                 %  kWh/yr
    % Montly Grid Purchases (kWh/yr):
    GPGM=(NWT)*PGROSS(xm(xmm))-LOAD(xm(xmm));                     % kW
    GPNM=(NWT)*PNET(xm(xmm))-LOAD(xm(xmm));                       % kW
    GPURGM(ii)=abs(sum(GPGM(find(GPGM<0))))/F;                    % kW/yr
    GPURNM(ii)=abs(sum(GPNM(find(GPNM<0))))/F;                    % kW/yr
    % Grid Sales (kWh/yr):
    GSALGM(ii)=abs(sum(GPGM(find(GPGM>0))))/F;                     % kW/yr
    GSALNM(ii)=abs(sum(GPNM(find(GPNM>0))))/F;                     % kW/y
    % Total Electrical Production (kW/yr):
    GTEPGM(ii)=WGM(ii)+GPURGM(ii);                                 % kW/yr
    GTEPNM(ii)=WNM(ii)+GPURNM(ii);                                 % kW/yr
    % Renewable Fraction:
    RFGM(ii)=100*WGM(ii)/GTEPGM(ii);                                % %
    RFNM(ii)=100*WNM(ii)/GTEPNM(ii);                                % %
end
%--------------------------------------------------------------------------
figure(np+8)
Y=[GTEPNM' WNM' LDM']/1e3;                                           % MWh
Y1=bar(Y);
Y1(1).LineWidth = 2;
Y1(1).FaceColor = [0.87 .53 .07];
Y1(1).EdgeColor = [0 0 0];
Y1(1).BarWidth= 0.95;
Y1(2).LineWidth = 2;
Y1(2).FaceColor = [0 .5 .5];
Y1(2).EdgeColor = [0 0 0];
Y1(2).BarWidth= 0.95;
Y1(3).LineWidth = 2;
Y1(3).FaceColor = [0.49 .18 .56];
Y1(3).EdgeColor = [0 0 0];
Y1(3).BarWidth= 0.95;
xlim([0 length(WNM)+1]);
%ylim([round(min(EW)) ceil(max(EW))]);
%ylim([0 50]);
title([PEL.WBASE,', ', PEL.WT],'fontname','tahoma','fontsize',18,'fontweight','light','LineWidth',2);
set(gca,'xtick',1:12,...
 'xticklabel',{'Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'},...
 'fontname','tahoma','fontsize',18,'fontweight','light','LineWidth',2);
ylabel('Energía (MWh)','fontsize',18,'fontname','tahoma','FontAngle','normal','fontweight','normal','rotation',90,...
    'HorizontalAlignment','center');
xlabel('Mes','fontsize',18,'fontname','tahoma','FontAngle','normal','fontweight','normal',...
    'HorizontalAlignment','center');
h = legend('Producción total de energía','Producción de energía eólica','Consumo','Location','north','Orientation','vertical');
title(h,[PELERATE ' ( ' num2str(PEL.yy,'%.0f') ' )']);
set(h,'LineWidth',2,'fontsize',14);
grid;
%--------------------------------------------------------------------------
% Grid Purchase and Grid Sales:
%--------------------------------------------------------------------------
figure(np+9)
Y=[GPURNM' GSALNM']/1e3;                                       % MWh
Y1=bar(Y);
Y1(1).LineWidth = 2;
Y1(1).FaceColor = [0 .5 .5];
Y1(1).EdgeColor = [0 0 0];
Y1(1).BarWidth= 0.95;
Y1(2).LineWidth = 2;
Y1(2).FaceColor = [0.49 .18 .56];
Y1(2).EdgeColor = [0 0 0];
Y1(2).BarWidth= 0.95;
xlim([0 length(WNM)+1]);
%ylim([round(min(EW)) ceil(max(EW))]);
...
xlabel('Mes','fontsize',18,'fontname','tahoma','FontAngle','normal','fontweight','normal',...
    'HorizontalAlignment','center');
h = legend('Compra de energía','Venta de energía','Location','north','Orientation','horizontal');
title(h,[PELERATE ' ( ' num2str(PEL.yy,'%.0f') ' )']);
set(h,'LineWidth',2,'fontsize',14);
grid;
%--------------------------------------------------------------------------
...
set(gca,'xtick',1:12,...
 'xticklabel',{'Ene','Feb','Mar','Abr','May','Jun','Jul','Ago','Sep','Oct','Nov','Dic'},...
 'fontname','tahoma','fontsize',18,'fontweight','light','LineWidth',2);
ylabel('Fracción de Energía Renovable (%)','fontsize',18,'fontname','tahoma','FontAngle','normal','fontweight','normal','rotation',90,...
    'HorizontalAlignment','center');
xlabel('Mes','fontsize',18,'fontname','tahoma','FontAngle','normal','fontweight','normal',...
    'HorizontalAlignment','center');
h = legend('E_b_r_u_t_a','E_n_e_t_a','Location','north','Orientation','horizontal');
title(h,[PELERATE ' ( ' num2str(PEL.yy,'%.0f') ' )']);
set(h,'LineWidth',2,'fontsize',14);
grid;
%--------------------------------------------------------------------------
